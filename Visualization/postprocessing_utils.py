...
def ESS(means, do_log, verbose=True):
    if not HAS_RPY2:
        raise ImportError("package rpy2 required to use ESS()")
    if do_log:
...
def geweke(means, do_log, split_l=0.1, split_r=0.5):
    if not HAS_RPY2:
        raise ImportError("package rpy2 required to use geweke()")
    if do_log:
        means = np.log10(means)
...
def gelman(means, do_log):
    if not HAS_RPY2:
        raise ImportError("package rpy2 required to use gelman()")
    if do_log:
        means = np.log10(means)
...
def calc_contours(x_accepted, y_accepted, clevels, which_params, size=1000, do_logx=False, do_logy=False, xrange=None, yrange=None):
    ...
    if "Sf" in which_params and "Sb" in which_params:
        ...
        cZ = mu_eff(cx, cy)
        # TODO: support cx+cy simultaneously for TRTS
        # cZ = (cx+cy)
