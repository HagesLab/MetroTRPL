...
def check_approved_param(new_p, param_info):
    ...
    if 'tauN' in order:
        checks["taun_size"] = (np.abs(new_p[order.index('tauN')] - np.log10(ucs.get('tauN', 1))) > -1)
    else:
        checks["taun_size"] = True

    if 'tauP' in order:
        checks["taup_size"] = (np.abs(new_p[order.index('tauP')] - np.log10(ucs.get('tauP', 1))) > -1)
    else:
        checks["taup_size"] = True
