...
* mpi4py 3.1.5 - for parallelized parallel tempering
...
4. Run the Metropolis sampler using main.py. Usage: `python main.py` while within the MetroTRPL folder. This outputs a log file detailing the random walks' progress and outputs an Ensemble object containing the log-likelihoods and locations of all visited states.
...
2. **param_info** - material parameter information
   ...
   * unit_conversions - Unit conversion factors for each parameter. Used by carrier transport models to convert from the common unit system used by material parameters (e.g. ns for lifetimes, cm^2/V/s for mobilities) to the nm/V/ns unit system used by the numerical solver.
...
4. **MCMC_fields** - settings controlling the MCMC algorithm itself.
   ...
   * rtol, atol, hmax - Solver tolerances and adaptive stepsize. See the solve_ivp docs for more details.
