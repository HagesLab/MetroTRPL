...
class EnsembleTemplate():
    ...
    param_indexes: dict[str, int] # Map of material parameter names and the order they appear in state arrays
    ...
    def one_sim_likelihood(self, meas_index, state, MCMC_fields, logger, verbose):
        ...
        if meas_type == "pa":
            ...
            tSteps, sol, success = self.converge_simulation(meas_index, state, iniPar, MCMC_fields, logger, verbose)
        if not success:
            likelihood = -np.inf
            return likelihood
        ...
        try:
            if irf_convolution is not None and irf_convolution[meas_index] != 0:
                if verbose and logger is not None:
                    logger.info(f"Convolving with wavelength {irf_convolution[meas_index]}")
                ...
                sol, times_c, vals_c, uncs_c = post_conv_trim(tSteps, sol, self.times[meas_index], self.vals[meas_index], self.uncs[meas_index])
        ...
        try:
            ...
            if not success:
                raise ValueError(f"{meas_index}: Simulation failed: too many negative vals")

            if n_fails > 0:
                logger.warning(f"{meas_index}: {n_fails} / {len(sol)} non-positive vals")
        ...
        if meas_type == "pa":
            ...
            try:
                ...
                if np.isnan(likelihood):
                    raise ValueError(f"{meas_index}: Simulation failed: invalid likelihood")
            except ValueError as e:
                logger.warning(e)
                likelihood = -np.inf
        return likelihood

    def converge_simulation(self, meas_index, state, iniPar, MCMC_fields, logger=None, verbose=True):
        """
        ...
        try:
            t_steps, sol = self.do_simulation(meas_index, state, iniPar, MCMC_fields)
        except ValueError as e:
            success = False
        ...
        if verbose and logger is not None:
            logger.info(f"{meas_index}: Simulation complete t {t_steps[0]}-{t_steps[-1]}")

        return t_steps, sol, success
    
    def do_simulation(self, meas_index, state, iniPar, MCMC_fields):
        """ Set up one simulation. """
        rtol = MCMC_fields.get("rtol", None)
