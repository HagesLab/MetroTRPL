...
def join_chains(dirname, exclude=None):
    dirname = os.path.join("bay_outputs", dirname)
    ...
    for param in names:
        setattr(base_MS.H, param, np.add.outer(np.zeros(len(MC_chains)), getattr(base_MS.H, param)))
        setattr(base_MS.H, f"mean_{param}", np.add.outer(np.zeros(len(MC_chains)), getattr(base_MS.H, f"mean_{param}")))
    ...
    for i, chain in enumerate(MC_chains):
        if i == 0: continue # Already done when making more room
        ...
        loglikelihood[i] = next_MS.H.loglikelihood
