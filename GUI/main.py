...
events = {"key": {"escape": "<Escape>", "enter": "<Return>"}, "click": {"left": "<Button-1>", "right": "<Button-3>"}}
...
class Plot:
    ...
    def traceplot2d(figure: Figure, x_list: np.ndarray, y_list: np.ndarray, x_label: str, y_label: str, scale: str) -> None:
        axes = figure.add_subplot()
        axes.plot(x_list, y_list)
        axes.plot(x_list[0], y_list[0], marker=".", linestyle=" ", color=GREEN, label="Start", markersize=10)
        axes.plot(x_list[-1], y_list[-1], marker=".", linestyle=" ", color=RED, label="End", markersize=10)
    ...
    def histogram2d(figure: Figure, x_list: np.ndarray, y_list: np.ndarray, x_label: str, y_label: str, scale: str) -> None:
        axes = figure.add_subplot()
...
class Window:
    class Popup:
        def __init__(self, master: tk.Tk, width: int, height: int, text: str, colors: tuple[str, str]) -> None:
            ...
            self.widget.title("")
            label = tk.Label(master=self.widget, width=width, height=height, text=text, background=colors[0], foreground=colors[1])
            label.pack()
    ...
    class Panel:
        def __init__(self, master: tk.Tk, width: int, height: int, color: str) -> None:
            self.widget = tk.Frame(master=master, width=width, height=height, background=color, border=4, relief="raised")
    ...
    def __init__(self, width: int, height: int, title: str) -> None:
        ...
        self.mini_panel.widgets["chart menu"] = tk.OptionMenu(self.mini_panel.widget, self.chart_type, "select")
        menu: tk.Menu = self.mini_panel.widgets["chart menu"]["menu"]
        menu.delete(0)
        menu.add_checkbutton(label="1D Trace Plot", onvalue="1D Trace Plot", offvalue="1D Trace Plot", variable=self.chart_type, command=self.chartselect)
        menu.add_checkbutton(label="2D Trace Plot", onvalue="2D Trace Plot", offvalue="2D Trace Plot", variable=self.chart_type, command=self.chartselect)
        menu.add_checkbutton(label="1D Histogram", onvalue="1D Histogram", offvalue="1D Histogram", variable=self.chart_type, command=self.chartselect)
        menu.add_checkbutton(label="2D Histogram", onvalue="2D Histogram", offvalue="2D Histogram", variable=self.chart_type, command=self.chartselect)
        ...
        load_button = tk.Button(master=self.mini_panel.widget, width=10, text="Load File", background=BLACK, foreground=WHITE, command=self.loadfile, border=4)
        load_button.place(x=200, y=40, anchor="s")
        self.mini_panel.widgets["load button"] = load_button
        graph_button = tk.Button(master=self.mini_panel.widget, width=10, text="Graph", background=BLACK, foreground=WHITE, command=self.drawchart, border=4)
        ...
        data_label = tk.Label(master=self.base_panel.widget, textvariable=self.file, width=138, height=11, background=LIGHT_GREY, relief="sunken", border=2, anchor="nw", justify="left")
    ...
    def loadfile(self) -> None:
        file_name = filedialog.askopenfilename(filetypes=[("Pickle File", "*.pik")], title="Select File", initialdir=PICKLE_FILE_LOCATION)
        if file_name == "":
            return
        self.base_panel.setvar("data label", self.base_panel.getvar("data label") + "\nText can also be appened (or deleted).")
        ...
        for key in metrostate.param_info["names"]:
            if metrostate.param_info["active"][key]:
                self.data[key] = {0: metrostate.H.__getattribute__(key)[0], 1: metrostate.H.__getattribute__(f"mean_{key}")[0]}
        ...
        match key:=self.side_panel.state:
            case "1D Trace Plot":
                ...
                if scale == "Logarithmic":
                    ...
                    scale = "linear"
                Plot.traceplot1d(self.chart.figure, self.data[value][accepted], title, scale, *hline)
            case "2D Trace Plot":
                name = self.side_panel.widgets["variable 1"]["textvariable"]
                ...
                if scale == "Logarithmic":
                    ...
                    scale = "linear"
                Plot.traceplot2d(self.chart.figure, self.data[x_val][True], self.data[y_val][True], x_val, y_val, scale)
            case "1D Histogram":
                name = self.side_panel.widgets["variable 1"]["textvariable"]
                value = self.widget.getvar(name)
                name = self.side_panel.widgets["scale"]["textvariable"]
                ...
                if scale == "Logarithmic":
                    ...
                    scale = "linear"
                Plot.histogram1d(self.chart.figure, self.data[value][True], f"Accepted {value}", scale)
            case "2D Histogram":
                name = self.side_panel.widgets["variable 1"]["textvariable"]
                x_val = self.widget.getvar(name)
                name = self.side_panel.widgets["variable 2"]["textvariable"]
                ...
                if scale == "Logarithmic":
                    ...
                    scale = "linear"
                Plot.histogram2d(self.chart.figure, self.data[x_val][True], self.data[y_val][True], x_val, y_val, scale)
        self.chart.canvas.draw()

window = Window(1000, 800, APPLICATION_NAME)
window.bind(events["key"]["escape"], lambda code: exit())

widgets = window.side_panel.widgets
panel = window.side_panel.widget
...
locations = [{"x": 20, "y": 20, "anchor": "nw"}, {"x": 200, "y": 20, "anchor": "n"}, {"x": 380, "y": 20, "anchor": "ne"},
             {"x": 20, "y": 48, "anchor": "nw"}, {"x": 200, "y": 48, "anchor": "n"}, {"x": 380, "y": 48, "anchor": "ne"},
             {"x": 20, "y": 88, "anchor": "nw"}, {"x": 200, "y": 88, "anchor": "n"}, {"x": 380, "y": 88, "anchor": "ne"},
             {"x": 20, "y": 116, "anchor": "nw"}, {"x": 200, "y": 116, "anchor": "n"}, {"x": 380, "y": 116, "anchor": "ne"}]
...
menu.add_checkbutton(label="Logarithmic", onvalue="Logarithmic", offvalue="Logarithmic", variable=scale)
window.side_panel.addstate("1D Trace Plot", [(widgets["label 1"], locations[0]), (widgets["label 4"], locations[1]), (widgets["label 3"], locations[2]),
                                             (widgets["variable 1"], locations[3]), (widgets["accepted"], locations[4]), (widgets["scale"], locations[5]),
                                             (widgets["label 5"], locations[6]), (widgets["entry"], locations[9])])
window.side_panel.addstate("2D Trace Plot", [(widgets["label 1"], locations[0]), (widgets["label 2"], locations[1]), (widgets["label 3"], locations[2]),
                                             (widgets["variable 1"], locations[3]), (widgets["variable 2"], locations[4]), (widgets["scale"], locations[5])])
window.side_panel.addstate("1D Histogram", [(widgets["label 1"], locations[0]), (widgets["label 3"], locations[1]), (widgets["variable 1"], locations[3]),
                                            (widgets["scale"], locations[4])])
window.side_panel.addstate("2D Histogram", [(widgets["label 1"], locations[0]), (widgets["label 2"], locations[1]), (widgets["label 3"], locations[2]),
                                            (widgets["variable 1"], locations[3]), (widgets["variable 2"], locations[4]), (widgets["scale"], locations[5])])

window.base_panel.setvar("data label", "Text can be inserted here.")
