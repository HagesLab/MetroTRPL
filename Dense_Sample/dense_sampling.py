...
def random_grid(min_X, max_X, do_log, num_samples):
    ...
    for i in range(num_params):
        if min_X[i] == max_X[i]:
            ...
            if do_log[i]:
                grid[:, i] = 10 ** np.random.uniform(
                    np.log10(min_X[i]), np.log10(max_X[i]), (num_samples,)
                )
            else:
...
def simulate(
    ...
    units = np.array(
        [param_info["unit_conversions"].get(param, 1) for param in param_info["names"]],
        dtype=float,
    )
    param_indexes = {
    ...
    where_sfs = {
        s_name: i
        for i, s_name in enumerate(param_info["names"])
    ...
    for ic_num in range(sim_params["num_meas"]):
        ...
        for i, mp in enumerate(X):
            ...
            P[i] -= np.sum(
                ...
                / (sim_flags["current_sigma"][meas_type] ** 2 + 2 * uncs_c**2)
            )
...
def modify_scale_factors(param_info, sim_flags):
    ...
    for name in param_info["names"]:
        if name.startswith("_s"):
            param_info["prior_dist"][name] = (
                param_info["init_guess"][name] / spread,
                param_info["init_guess"][name] * spread,
            )
...
def bayes(N, P, init_params, sim_params, e_data, sim_flags, param_info, logger=None):
    ...
    if sim_flags.get("scale_factor", None) is not None:
        modify_scale_factors(param_info, sim_flags)

    min_X = np.array(
        [
            param_info["prior_dist"][name][0]
            if param_info["active"][name]
            else param_info["init_guess"][name]
            for name in param_info["names"]
        ]
    )
    max_X = np.array(
        [
            param_info["prior_dist"][name][1]
            if param_info["active"][name]
            else param_info["init_guess"][name]
            for name in param_info["names"]
        ]
    ...
    if logger is not None:
        logger.info(f"Initializing {len(X)} random samples")
        logger.info(f"First three samples: {X[0:3]}")

    param_info["trial_move"] = np.array([param_info["trial_move"][param]
                                         for param in param_info["names"]], dtype=float)
    if isinstance(sim_flags["likel2move_ratio"], dict):
        sim_flags["current_sigma"] = {m:max(param_info["trial_move"]) * sim_flags["likel2move_ratio"][m]
                                                    for m in sim_params["meas_types"]}
    else:
        sim_flags["current_sigma"] = {m:max(param_info["trial_move"]) * sim_flags["likel2move_ratio"]
                                                    for m in sim_params["meas_types"]}
    ...
    if logger is not None:
        logger.info(
            "Total tEvol time: {}, avg {}".format(solver_time, np.mean(solver_time))
        )
        logger.info(
            "Total err_sq time (temperatures and mag_offsets): {}, avg {}".format(
                err_sq_time, np.mean(err_sq_time)
            )
        )
