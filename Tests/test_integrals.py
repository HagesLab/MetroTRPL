...
import sys
sys.path.append("..")
from forward_solver import calculate_PL, calculate_TRTS, integrate_1D
# from sim_utils import check_threshold
...
class TestUtils(unittest.TestCase):
    ...
    def test_Solution_2D(self):
        ...
        num_nodes = 128
        num_tsteps = 10
        N = np.ones((num_tsteps, num_nodes))
        P = np.ones((num_tsteps, num_nodes))
        ...
        pl = calculate_PL(dx, N, P, param_info["init_guess"]["ks"], param_info["init_guess"]["n0"], param_info["init_guess"]["p0"])
        np.testing.assert_equal(np.zeros(num_tsteps), pl)

        # Some PL
        initial_guesses = {"ks":1, 'n0':0, 'p0':0}
        param_info["init_guess"] = initial_guesses
        P = np.ones((num_tsteps, num_nodes)) * 10
        pl = calculate_PL(dx, N, P, param_info["init_guess"]["ks"], param_info["init_guess"]["n0"], param_info["init_guess"]["p0"])
        expected_PL = initial_guesses["ks"] * (N * P - initial_guesses["n0"]*initial_guesses["p0"])
        # Like integrating, since dx=1
        np.testing.assert_almost_equal(expected_PL[:,0] * num_nodes, pl)
        ...
        param_info["init_guess"] = initial_guesses
        P = np.ones((num_tsteps, num_nodes)) * 10
        trts = calculate_TRTS(dx, N, P, param_info["init_guess"]["mu_n"], param_info["init_guess"]["mu_p"], param_info["init_guess"]["n0"], param_info["init_guess"]["p0"])
        expected_TRTS = q_C * (initial_guesses["mu_n"] * (N - initial_guesses["n0"]) +
                       initial_guesses["mu_p"] * (P - initial_guesses["p0"]))
        np.testing.assert_almost_equal(expected_TRTS[:,0] * num_nodes, trts)
